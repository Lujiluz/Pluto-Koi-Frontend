  ...
  return createPortal(
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[60] p-4 sm:p-6" onClick={onClose}>
      ...
      <div className="relative w-full max-w-4xl transform overflow-hidden rounded-xl bg-white shadow-2xl transition-all max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        {/* Header (ditambahkan padding kanan agar tidak tertutup tombol X) */}
        <div className="border-b border-gray-200 px-6 py-4 pr-14 bg-gradient-to-r from-primary-50 to-white">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-2xl font-bold text-gray-900">Leaderboard Lelang</h2>
              <p className="text-sm text-gray-600 mt-1">{auction.itemName}</p>
            </div>
            {/* TOMBOL X LAMA DIHAPUS DARI SINI */}
          </div>
