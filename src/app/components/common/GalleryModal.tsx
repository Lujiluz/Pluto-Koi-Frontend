...
import { createPortal } from "react-dom"; // <-- TETAP ADA
...
export default function GalleryModal({ isOpen, onClose, gallery }: GalleryModalProps) {
  ...
  const [isMounted, setIsMounted] = useState(false); // <-- TETAP ADA
  ...
  return createPortal(
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[60] overflow-y-auto p-4 sm:p-6 md:p-8" onClick={handleOverlayClick}>
      <div className="min-h-full flex justify-center items-center">
        <div
          className="bg-white rounded-2xl relative animate-in fade-in-0 zoom-in-95 duration-200
  w-full max-w-sm sm:max-w-md md:max-w-lg xl:max-w-4xl my-4 h-auto no-scrollbar"
          onClick={(e) => e.stopPropagation()}
        >
          <button onClick={onClose} className="absolute top-4 right-4 p-2 z-10 bg-white/50 backdrop-blur-sm hover:bg-gray-100 rounded-full transition-colors">
            <X size={24} />
          </button>

          {/* Header */}
          <div className="flex items-center justify-between pt-6 pr-14 pl-6 border-b">
            <div>
              <h2 className="text-2xl font-bold text-black">{gallery.galleryName}</h2>
              <div className="flex items-center space-x-2 text-xs md:text-sm text-gray-600 mt-4 pb-4 md:pb-6">
